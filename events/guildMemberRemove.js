...
async function invoke(member) {
		...
		createEmbeds: function(type) {
			if (type === "join") {
				const embed = new EmbedBuilder()
					...
					.setDescription(`Bir maceracı aramıza katıldı! Sunucumuzda maceracı sayısı **${member.guild.memberCount}.** kişi olan **${general.getMemberInfo()['username']}** ile beraber yükseldi!`)
				...
				const embed = new EmbedBuilder()
					.setAuthor({
						name: "Maceracı kayıplara karıştı!", 
						iconURL: member.displayAvatarURL( { dynamic: true } )
					})
	...
	const message = await general.createEmbeds("leave");
	const channel = await general.getSendedChannel(sendedChannel)
